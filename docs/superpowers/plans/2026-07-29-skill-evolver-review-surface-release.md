...
- A candidate-session link is the session-level evidence idempotence
  authority. When an existing link names the same candidate, commit suppresses
  the entire evidence insertion loop even after 90-day aggregation has deleted
  the individual evidence rows. Only a first distinct session records at least
  one evidence row before inserting its link; the retained link prevents raw
  evidence or aggregate-count reintroduction until the 180-day session
  boundary.
...
- an existing same-candidate session link suppresses every evidence insertion
  after 90-day aggregation/redaction, so the aggregate count remains unchanged
  through maintenance and the 180-day link boundary;
